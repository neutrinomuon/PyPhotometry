...
class create_database_filters(object):
    ...
    def read_spectra( self, path_Vega=None,path1Vega=None,path_Sun=None,path1Sun=None,path2Sun=None,path_BD=None, verbose=False ):
        ...
        if path_Vega == None:
            ...
            # Get the base directory path of the package
            path1Vega = package_dist.location + '/' + package_name + '/data/calibration_stars/Vega.dat'
        ...
        o = Filters( )
        o.ReadCalibrationStars(  path_Vega=path_Vega,path1Vega=path1Vega,path_Sun=path_Sun,path1Sun=path1Sun,path2Sun=path2Sun,path_BD=path_BD )
    ...
    def evaluate_photometry( self, l, f, ind_filters=None, plot=False  ):
        ...
        if plot:
            ...
            for i in enumerate(self.T_lambda[0,ind_filters]):
                #print(i)
                print( ind[i[0]] , self.name_fil[ind[i[0]]], self.lamb_eff[ind[i[0]]] )
...
class Filters( object ):
    ...
    def ReadCalibrationStars( self,path_Vega=None,path1Vega=None,path_Sun=None,path1Sun=None,path2Sun=None,path_BD=None,verbose=0 ):
# ! *** Read calibration stars ************************************************
...
# *** Interpolate/Extrapolate boundaries *********************************************
        # 2.99953700e+06        
        lamb_extended = np.arange( 3.2e6,7.1e6,1.0e5 )

        lamb_extended = np.concatenate( [self.lambVega,lamb_extended] )
        # self.fluxVega = np.interp( lamb_extended,self.lambVega,self.fluxVega )
        # self.lambVega = lamb_extended
        
        # Perform linear interpolation with linear extrapolation
        flux_extended = np.interp( lamb_extended,self.lambVega[:-1],self.fluxVega[:-1], left=np.nan, right=np.nan )
        # Replace NaNs with your desired extrapolation values (e.g., linear extrapolation)
        flux_extended[np.isnan(flux_extended)] = np.interp( lamb_extended[np.isnan(flux_extended)],self.lambVega[:-1],self.fluxVega[:-1] )
        
        # Calculate the slope for extrapolation
        # slope = (self.fluxVega[-1] - self.fluxVega[-2]) / (self.lambVega[-1] - self.lambVega[-2])
        # print(slope)

        # # Extrapolate values outside the boundaries linearly
        # for i in range(len(lamb_extended)):
        #     if lamb_extended[i] < self.lambVega[0]:
        #         flux_extended[i] = self.fluxVega[0] + slope * (lamb_extended[i] - self.lambVega[0])
        #     elif lamb_extended[i] > self.lambVega[-1]:
        #         flux_extended[i] = self.fluxVega[-1] + slope * (lamb_extended[i] - self.lambVega[-1])
        
        self.fluxVega = flux_extended
        self.lambVega = lamb_extended
...
if __name__ == '__main__':
    ...
    # Get the base directory path of the package
    path = package_dist.location + '/' + package_name + '/data/templates/bc2003_hr_m62_chab_ssp_160.spec'
    ...
    l_ = np.arange(100.,10020001.,20., dtype=float)
    #print(l_[0],l_[-1])
    f_ = np.interp(l_,l,f)
    l = l_
    f = f_
